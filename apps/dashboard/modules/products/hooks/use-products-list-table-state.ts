...
import { PRODUCT_TABLE_SORT_IDS } from '@/modules/common/utils/data-table-sort-allowlists';
...
export function useProductsListTableState() {
  ...
  const onSortingChange: OnChangeFn<SortingState> = useCallback((updater) => {
    setSorting((previous) => {
      const next = typeof updater === 'function' ? updater(previous) : updater;
      return normalizeSortingState(next, PRODUCT_TABLE_SORT_IDS);
    });
    setPagination((previous) => ({ ...previous, pageIndex: 0 }));
  }, []);
