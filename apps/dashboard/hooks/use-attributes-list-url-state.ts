...
function buildSearchParamsFromState(
  ...
  appendSortingToSearchParams(params, sorting, ATTRIBUTE_TABLE_SORT_IDS);
  return params;
}

export function useAttributesListUrlState() {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const globalFilter = useMemo(() => searchParams.get('q')?.trim() ?? '', [searchParams]);
  const sorting = useMemo(
    () => sortingStateFromUrl(searchParams, ATTRIBUTE_TABLE_SORT_IDS),
    [searchParams],
  );
  ...
  const onPaginationChange: OnChangeFn<PaginationState> = useCallback(
    (updater) => {
      ...
      replaceUrl(buildSearchParamsFromState(next, globalFilter, sorting));
    },
  ...
  const onGlobalFilterChange = useCallback(
    (value: string) => {
      replaceUrl(
        buildSearchParamsFromState({ pageIndex: 0, pageSize: pagination.pageSize }, value, sorting),
      );
    },
