...
function buildSearchParamsFromState(
  ...
  appendSortingToSearchParams(params, sorting, SIZE_TABLE_SORT_IDS);
  return params;
}

export function useSizesListUrlState() {
  ...
  const columnFilters = useMemo<ColumnFiltersState>(() => {
    ...
    if (!q) return [];
    return [{ id: 'label', value: q }];
  }, [searchParams]);

  const sorting = useMemo(
    () => sortingStateFromUrl(searchParams, SIZE_TABLE_SORT_IDS),
    [searchParams],
  );
