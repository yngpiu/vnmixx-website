...
CREATE TABLE `banners` (
    `id` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT,
    `placement` ENUM('HERO_SLIDER', 'FEATURED_TILE', 'PROMO_STRIP') NOT NULL,
...
CREATE TABLE `product_variants` (
    ...
    `compare_at_price` INTEGER UNSIGNED NULL,
...
CREATE TABLE `orders` (
    ...
    `customer_id` INTEGER UNSIGNED NOT NULL,
    `status` ENUM('PENDING_CONFIRMATION', 'PROCESSING', 'AWAITING_SHIPMENT', 'SHIPPED', 'DELIVERED', 'CANCELLED') NOT NULL DEFAULT 'PENDING_CONFIRMATION',
...
CREATE TABLE `order_status_histories` (
    `id` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT,
    `order_id` INTEGER UNSIGNED NOT NULL,
    `status` ENUM('PENDING_CONFIRMATION', 'PROCESSING', 'AWAITING_SHIPMENT', 'SHIPPED', 'DELIVERED', 'CANCELLED') NOT NULL,
    `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0),
...
CREATE TABLE `sepay_transactions` (
    ...
    `matched_order_code` VARCHAR(64) NULL,
...
-- AddForeignKey
ALTER TABLE `banners` ADD CONSTRAINT `banners_category_id_fkey` FOREIGN KEY (`category_id`) REFERENCES `categories`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
